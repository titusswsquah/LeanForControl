...
imports: power-bounded marginal, `eq:Finf-spec`, and the reduced
`fact:dare-strong`), the run from the prior is attracted to the strong
...
dichotomy's GAS half): under C1 — with the deck's imports
(power-bounded marginal, `eq:Finf-spec`, reduced `fact:dare-strong`) —
the run from the prior is attracted to the strong solution **iff**
C2w. -/
theorem strong_attraction_iff_C2w (hC1 : S.C1)
  ...
  ⟨fun hconv => S.necessity hS hconv,
    fun hC2w => S.sufficiency_tendsto hC1 hC2w hS hcm hPB hFs himp⟩
