...
/-- **Prior-free window growth**: under C1 alone, the horizon-`T`
Riccati form itself grows linearly in the number of full windows,
weighted by the antistable block of the initial state. -/
theorem exists_window_ric_growth (hC1 : Sys.C1) :
    ∃ β : ℝ, 0 < β ∧ ∀ (e₀ : Fin n₁ ⊕ Fin n₂ → ℝ) (J T : ℕ), 1 ≤ J →
      (n₁ + n₂) * J ≤ T →
      β * J * ‖blk₂ e₀‖ ^ 2 ≤ quadForm (Sys.lq.ric T) e₀ := by
  rcases Nat.eq_zero_or_pos (n₁ + n₂) with hM0 | hM1
  · refine ⟨1, one_pos, fun e₀ J T _ hJT => ?_⟩
    have hn₂ : n₂ = 0 := by omega
    subst hn₂
    have h0 : blk₂ e₀ = 0 := Subsingleton.elim _ _
    rw [h0, norm_zero]
    have h1 := (Sys.lq.ric_posSemidef T).quadForm_nonneg e₀
  ...
  · obtain ⟨α, hα, hcoer⟩ := Sys.exists_window_coercivity hC1
    ...
    refine ⟨α * cB, by positivity, fun e₀ J T hJ1 hJT => ?_⟩
    ...
    have h5 : ∑ j ∈ Finset.range J,
      ...
      _ ≤ Sys.lq.cost e₀ ω T := by
            ...
            _ ≤ T := hJT
    have h6 : Sys.lq.cost e₀ ω T = quadForm (Sys.lq.ric T) e₀ := by
